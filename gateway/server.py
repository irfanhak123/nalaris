...
class GatewayHandler(BaseHTTPRequestHandler):
    ...
    def _handle_get(self, parsed) -> bool:
        ...
        if path == "/api/chat/stream":
            ...
            from gateway.sse import stream_from_queue
    ...
    def _handle_post(self, parsed) -> bool:
        ...
        if path == "/api/session/new":
            ...
            session = store.create_session(session_id, workspace, profile, title)
            return json_response(self, {"session": session})
        ...
        if path == "/api/chat/start":
            try:
                ...
                result = bridge.start_chat(session_id, message, workspace, profile, store=store)
    ...
    def _handle_get_panel_session(self) -> bool:
        ...
        panel_session_file = Path("/tmp/panel-gateway-session-id")
    ...
    def _handle_set_panel_session(self) -> bool:
        ...
        if not session_id:
            return json_response(self, {"error": "session_id required"}, 400)
        try:
            Path("/tmp/panel-gateway-session-id").write_text(session_id)
...
def run_server(host: str = HOST, port: int = PORT):
    ...
    from gateway.agent_bridge import AgentBridge
